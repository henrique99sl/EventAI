...
def test_update_user_404(client, admin_token):
    resp = client.put(
        "/users/99999", headers=auth_header(admin_token), json={"username": "newname"}
    )
...
def test_create_user_duplicate_email_username(client):
    ...
    client.post(
        "/users", json={"username": username, "email": email, "password": "Senha1234"}
    )
    # Tenta criar com mesmo email
    resp1 = client.post(
        "/users",
        json={"username": "otheruser", "email": email, "password": "Senha1234"},
    )
...
def test_get_me(client):
    ...
    login = client.post("/login", json={"email": email, "password": "Senha1234"})
...
def test_update_event_404(client, admin_token):
    resp = client.put(
        "/events/99999", headers=auth_header(admin_token), json={"name": "novo"}
    )
    assert resp.status_code == 404
...
def test_update_event_forbidden(client):
    ...
    client.post(
        "/users", json={"username": username1, "email": email1, "password": "Senha1234"}
    )
    login1 = client.post("/login", json={"email": email1, "password": "Senha1234"})
    ...
    client.post(
        "/users", json={"username": username2, "email": email2, "password": "Senha1234"}
    )
    login2 = client.post("/login", json={"email": email2, "password": "Senha1234"})
    ...
    resp3 = client.put(f"/events/{event_id}", headers=headers2, json={"name": "novo"})
