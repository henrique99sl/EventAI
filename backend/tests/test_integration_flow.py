def test_full_flow(client):
    ...
    resp = client.post(
        "/login",
        json={"email": "flowuser@gmail.com", "password": "SenhaF0rte!"},
    )
    ...
    venue_resp = client.post(
        "/venues",
        json={"name": "Venue Flow", "address": "Rua Flow"},
    ...
    events = (
        resp.get_json()
        if isinstance(resp.get_json(), list)
        else resp.get_json().get("events", [])
    )
