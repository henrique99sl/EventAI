...
def test_bulk_create_users_and_events(client):
    ...
    tokens = []
    for i in range(20):
        resp = client.post("/users", json={"username": f"user{i}", "email": f"user{i}@mail.com", "password": "StrongPass1"})
        assert resp.status_code == 201
        login = client.post("/login", json={"email": f"user{i}@mail.com", "password": "StrongPass1"})
    ...
    for token in tokens:
        ...
        for j in range(5):
            resp2 = client.post("/events", json={"name": f"E{j}", "date": "2025-12-12", "venue_id": venue_id}, headers=headers)
...
def test_events_pagination(client, user_token):
    ...
    venue_id = resp.get_json()["id"]
    for i in range(30):
        client.post("/events", json={"name": f"E{i}", "date": "2025-12-12", "venue_id": venue_id}, headers=headers)
