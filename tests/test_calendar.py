...
def test_calendar_returns_events_in_interval(client, user_token):
    ...
    assert set(names) == {f"Event 0", f"Event 1", f"Event 2"}
...
def test_calendar_excludes_events_outside_interval(client, user_token):
    ...
    today = date.today()
    resp = client.get(f"/events/calendar?start={today.isoformat()}&end={(today + timedelta(days=7)).isoformat()}", headers=headers)
    assert resp.status_code == 200
    events = resp.get_json()
...
def test_calendar_event_structure(client, user_token):
    ...
    resp = client.get(f"/events/calendar?start={date.today().isoformat()}&end={date.today().isoformat()}", headers=headers)
    assert resp.status_code == 200
    events = resp.get_json()
    for ev in events:
        assert "id" in ev
